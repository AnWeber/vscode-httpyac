    ...
    if (isInRequestLine && httpRegion?.request) {
      ...
      if (httpyac.utils.isHttpRequest(httpRegion.request)) {
        ...
        result.push(...[
          ...
          { name: 'keepalive', description: '10 seconds, set to 0 to disable', kind: vscode.CompletionItemKind.Field },
          { name: 'QoS', description: 'the QoS used for subscribe or publish', kind: vscode.CompletionItemKind.Field },
